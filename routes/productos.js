...
router.get("/", (req, res) => {
  async function getTodos(){
    try{
      ...
      //res.send(`Estos son los productos guardados actualmente:${aux}`);
    }
    catch(error){
  ...
  async function getxId(){
    try{
      ...
      //PTO CON ID SOLICITADO ENCONTRADO
        ...
        //ptoId = JSON.stringify(ptoId, null, 2);
        //res.send(`Este es el producto con el id solicitado: <br>${ptoId} `);
        res.send(ptoId);
      }
    }
    catch(error){
